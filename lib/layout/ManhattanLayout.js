...
function getSegmentBendpoints(a, b, directions, source = null, target = null) {
  ...
  // special case when going left and down
  if ((b.x - a.x) < 0 && (b.y - a.y) > 0) {
    if (source && (source.type === 'bpmn:Task' || source.type === 'bpmn:CallActivity')) {
      ymid = b.y + target.height + 20 + 35
    } else {
      ymid = b.y + target.height + 20 + 45
    }
  }
  ...
  if (layoutStraight) {
    ...
    if (layoutTurn) {
      ...
      if (source && (source.type === 'bpmn:Task' || source.type === 'bpmn:CallActivity')) {
        var MIN_SEGMENT_LENGTH_Y = 10;
      } else {
        MIN_SEGMENT_LENGTH_Y = 18;
      }
      if (layoutHorizontal) {
        turnNextDirections = ymid === a.y;
        segmentEnd = {
          x: a.x + MIN_SEGMENT_LENGTH * (/l/.test(startDirection) ? -1 : 1),
          y: turnNextDirections ? ymid + MIN_SEGMENT_LENGTH_Y : ymid
        };
      } else {
        ...
        segmentEnd = {
          ...
          y: a.y + MIN_SEGMENT_LENGTH_Y * (/t/.test(startDirection) ? -1 : 1)
        };
      }
...
function getStartSegment(a, b, directions, source, target) {
  return getSegmentBendpoints(a, b, directions, source, target);
}
 ...
 */
function getSimpleBendpoints(a, b, directions, source) {

  if (source && source.type === 'bpmn:ExclusiveGateway') {
    var xmid = round(60 + a.x);
  } else {
	  var xmid = round(b.x - 30);
  }
  var ymid = round((b.y - a.y) / 2 + a.y);
 ...
 * @return {Array<Point>}
 */
function getBendpoints(a, b, directions, source = null, target = null) {
  ...
  if (isExplicitDirections(directions)) {
    var startSegment = getStartSegment(a, b, directions, source, target),
  ...
  return getSimpleBendpoints(a, b, directions, source);
}
 ...
 * @return {Array<Point>}
 */
export function connectPoints(a, b, directions, source, target) {

  var points = getBendpoints(a, b, directions, source, target);
...
export function connectRectangles(source, target, start, end, hints) {
  ...
  return connectPoints(startDocking, endDocking, directions, source, target);
}
