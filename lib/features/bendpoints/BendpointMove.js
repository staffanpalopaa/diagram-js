...
export default function BendpointMove(injector, eventBus, canvas, dragging, rules, modeling) {
  ...
  this.start = function(event, connection, bendpointIndex, insert) {
    ...
    // we validate two rules
    //
    // * connection.updateWaypoints - when moving or adding a bendpoint
    // * connection.reconnect - when reconnecting start or end
    var ruleType = type !== COMMAND_BENDPOINT_UPDATE ? 'connection.reconnect' : type;

    var allowed = rules.allowed(ruleType, {
  ...
  eventBus.on('bendpoint.move.hover', function(event) {
    ...
    allowed = context.allowed = rules.allowed(ruleType, {
      hover: hover,
